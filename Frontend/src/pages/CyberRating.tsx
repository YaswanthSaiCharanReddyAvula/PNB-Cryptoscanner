...
function getTier(score: number): { label: string; color: string; bg: string } {
  if (score > TIER_THRESHOLDS.elite)
    return { label: "Elite-PQC", color: GREEN, bg: "rgba(34,197,94,0.12)" };
  if (score >= TIER_THRESHOLDS.standard)
    return { label: "Standard", color: GOLD, bg: "rgba(251,188,9,0.12)" };
  return { label: "Legacy", color: RED, bg: "rgba(162,14,55,0.12)" };
}

function scaleScore(raw100: number): number {
  // Map a 0-100 raw score to 0-1000
...
function computeScoreFromScan(v1: any): number {
  ...
  const hasLegacy = tls.some((t: any) =>
    /1\.0|1\.1|ssl/i.test(String(t?.tls_version || ""))
  );
  const raw = typeof v1?.quantum_score?.score === "number"
    ? v1.quantum_score.score
    : 75;
  return scaleScore(Math.max(0, Math.min(100, raw - (hasLegacy ? 20 : 0))));
}

// ── Sample per-URL data (replaced after a real scan) ─────────────────────────

const SAMPLE_URL_SCORES = [
  { url: "https://pnbindia.com", score: 820 },
  { url: "https://netbanking.pnbindia.com", score: 760 },
  { url: "https://api.pnbindia.com/v1", score: 680 },
  { url: "https://corp.pnbindia.com", score: 540 },
  { url: "https://mobile.pnbindia.com", score: 100 },
  { url: "https://legacy.pnbindia.com/login", score: 0 },
];
...
const TIER_REFERENCE = [
  ...
  {
    status: "Standard",
    icon: <ArrowUpCircle size={16} color={GOLD} />,
    rating: "400 till 700",
    color: GOLD,
  },
  {
    status: "Legacy",
    icon: <CircleDot size={16} color={RED} />,
    rating: "< 400",
    color: RED,
  },
  {
    status: "Maximum Score after normalisation*",
    icon: null,
    rating: "1000",
    color: "#94a3b8",
  },
...
const TIER_CRITERIA = [
  ...
  },
  {
    ...
    level: "Moderate",
    compliance:
      "TLS 1.2 + legacy allowed · Key > 2048-bit · Minor cipher weaknesses",
  ...
  },
  {
    ...
    level: "Low",
    compliance:
      "TLS 1.0/1.1 enabled · Weak ciphers (CBC, 3DES) · Possible self-signed certs",
...
export default function CyberRating() {
  ...
  const [score1000, setScore1000] = useState(755); // default demo value
  const [urlScores, setUrlScores] = useState(SAMPLE_URL_SCORES);
  ...
  const tier = getTier(score1000);

  // Arc arc calculation: circumference = 2π × 85 ≈ 534
  const CIRC = 534;
  const dashArray = (score1000 / 1000) * CIRC;
  ...
  const fetchScore = async (v1FallbackPayload: any = null) => {
    try {
      ...
      setScore1000(scaleScore(raw));
    } catch {
      // keep default demo score
    }
    if (v1FallbackPayload) {
      setScore1000(computeScoreFromScan(v1FallbackPayload));
    }
  };
  ...
  useEffect(() => {
    if (results?.status === "completed" && !isScanning) {
      ...
      // Build per-URL rows from tls_results if available
      if (Array.isArray(results.tls_results) && results.tls_results.length) {
        const rows = results.tls_results.slice(0, 8).map((t: any) => ({
    ...
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-muted-foreground">
          Overall PQC-readiness score — maximum 1000 after normalisation
        </p>
      </motion.div>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Enter domain (e.g., pnbindia.com)"
              value={domain}
          ...
          <Button
            onClick={handleScan}
            disabled={isEmployee || !domain || isScanning}
        ...
        <div className="relative flex-shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span
              ...
              style={{ color: tier.color, transition: "color 0.8s ease" }}
            >
              {score1000}
            </span>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-3xl font-bold text-foreground">
              {score1000} <span className="text-muted-foreground font-normal text-xl">/ 1000</span>
            </span>
            <span
          ...
          <p className="text-muted-foreground text-sm max-w-md">
            ...
            security posture. {score1000 <= 700 && "Review risk factors and upgrade your TLS stack to improve."}
          </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-secondary/50">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Icon</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">PQC Rating For Enterprise</th>
              </tr>
            </thead>
            <tbody>
      ...
      >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-secondary/50">
                <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">URL</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">PQC Score</th>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-24 h-1.5 rounded-full bg-secondary overflow-hidden">
                          <div
                            ...
                            style={{ width: `${row.score / 10}%`, backgroundColor: t.color }}
                          />
                        </div>
                        <span className="font-semibold text-xs" style={{ color: t.color }}>
                          {row.score}
                        </span>
                      </div>
                    </td>
      ...
      >
        ...
        >
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-wide text-foreground text-left">
              Tier Compliance Criteria
            </h2>
            <p className="text-xs text-muted-foreground mt-0.5 text-left">
              Security requirements and remediation priorities per tier
            </p>
          </div>
          {tierOpen ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-secondary/50">
                      <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Tier</th>
                      <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Security Level</th>
                      <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Compliance Criteria</th>
                      <th className="text-left px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Priority / Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {TIER_CRITERIA.map((row, i) => (
                      <tr
                        key={i}
                        className="border-b border-border/50 hover:bg-secondary/30 transition-colors align-top"
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            ...
                            style={{
                              ...
                              backgroundColor: `${row.actionColor}18`,
                              border: `1px solid ${row.actionColor}35`,
                            }}
                          >
                            {row.level}
                          </span>
                        ...
                        <td className="px-6 py-4 text-xs font-medium" style={{ color: row.actionColor }}>
                          {row.action}
                        </td>
                      </tr>
                    ))}
      ...
      {/* ── Alert for critical score ───────────────────────────────── */}
      {score1000 < 400 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="rounded-xl border p-4 flex gap-3 items-start"
          style={{ borderColor: `${RED}50`, backgroundColor: `${RED}10` }}
        >
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Your score is below 400. TLS 1.0 / 1.1 or weak ciphers may be active. Upgrade your TLS stack and apply PQC recommendations immediately.
            </p>
          </div>
        </motion.div>
      )}
